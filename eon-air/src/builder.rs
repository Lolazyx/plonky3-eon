...
pub trait EonAirBuilder: Sized {
    ...
    fn when<I: Into<Self::Expr>>(&mut self, condition: I) -> FilteredEonAirBuilder<'_, Self> {
        FilteredEonAirBuilder {
            inner: self,
...
/// Helper macro to implement p3-air builder traits by delegating to EonAirBuilder.
///
...
/// `EonAirBuilder` implementation.
///
/// # Usage
///
/// ```rust,ignore
/// use eon_air::{EonAirBuilder, impl_p3_air_builder_traits};
///
/// struct MyBuilder { /* ... */ }
///
/// impl EonAirBuilder for MyBuilder {
...
macro_rules! impl_p3_air_builder_traits {
    ...
    ($builder_type:ty) => {
        impl $crate::AirBuilder for $builder_type {
            type F = <Self as $crate::EonAirBuilder>::F;
            type Expr = <Self as $crate::EonAirBuilder>::Expr;
            type Var = <Self as $crate::EonAirBuilder>::Var;
            type M = <Self as $crate::EonAirBuilder>::M;

            fn main(&self) -> Self::M {
                <Self as $crate::EonAirBuilder>::main(self)
            }

            fn is_first_row(&self) -> Self::Expr {
                <Self as $crate::EonAirBuilder>::is_first_row(self)
            }

            fn is_last_row(&self) -> Self::Expr {
                <Self as $crate::EonAirBuilder>::is_last_row(self)
            }

            fn is_transition_window(&self, size: usize) -> Self::Expr {
                <Self as $crate::EonAirBuilder>::is_transition_window(self, size)
            }

            fn assert_zero<I: Into<Self::Expr>>(&mut self, x: I) {
                <Self as $crate::EonAirBuilder>::assert_zero(self, x)
            }
        }

        impl $crate::AirBuilderWithPublicValues for $builder_type {
            type PublicVar = <Self as $crate::EonAirBuilder>::PublicVar;
            fn public_values(&self) -> &[Self::PublicVar] {
                <Self as $crate::EonAirBuilder>::public_values(self)
            }
        }

        impl $crate::PairBuilder for $builder_type {
            fn preprocessed(&self) -> Self::M {
                <Self as $crate::EonAirBuilder>::preprocessed(self)
            }
        }
        ...
        where
            Self::F: $crate::Field,
        {
            type EF = <Self as $crate::EonAirBuilder>::EF;
            type ExprEF = <Self as $crate::EonAirBuilder>::ExprEF;
            type VarEF = <Self as $crate::EonAirBuilder>::VarEF;

            fn assert_zero_ext<I>(&mut self, x: I)
            where
                I: Into<Self::ExprEF>,
            {
                <Self as $crate::EonAirBuilder>::assert_zero_ext(self, x)
            }
        }
        ...
        where
            Self::F: $crate::Field,
        {
            type MP = <Self as $crate::EonAirBuilder>::MP;
            type RandomVar = <Self as $crate::EonAirBuilder>::RandomVar;
            fn permutation(&self) -> Self::MP {
                <Self as $crate::EonAirBuilder>::permutation(self)
            }

            fn permutation_randomness(&self) -> &[Self::RandomVar] {
                <Self as $crate::EonAirBuilder>::permutation_randomness(self)
            }
        }
    };
