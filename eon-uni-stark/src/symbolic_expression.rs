    ...
    BinomialExtensionField<F, D>: ExtensionField<F>,
{
    ...
    /// This works for Fr, KoalaBear, Mersenne31, and any future field
...
mod tests {
    ...
    fn test_symbolic_expression_degree_multiple() {
        let constant_expr = SymbolicExpression::<Fr>::Constant(Fr::new(5));
        assert_eq!(
        ...
        let permutation_var = SymbolicExpression::Variable(SymbolicVariable::<Fr>::new(
        ...
        let public_var =
            SymbolicExpression::Variable(SymbolicVariable::<Fr>::new(Entry::Public, 4));
        assert_eq!(
        ...
        let challenge_var =
            SymbolicExpression::Variable(SymbolicVariable::<Fr>::new(Entry::Challenge, 5));
        assert_eq!(
        ...
        let is_first_row = SymbolicExpression::<Fr>::IsFirstRow;
        assert_eq!(
        ...
        let is_last_row = SymbolicExpression::<Fr>::IsLastRow;
        assert_eq!(
        ...
        let is_transition = SymbolicExpression::<Fr>::IsTransition;
        assert_eq!(
        ...
        let add_expr = SymbolicExpression::<Fr>::Add {
        ...
        let sub_expr = SymbolicExpression::<Fr>::Sub {
            x: Arc::new(variable_expr.clone()),
            y: Arc::new(preprocessed_var.clone()),
            degree_multiple: 1,
        };
        ...
        let neg_expr = SymbolicExpression::<Fr>::Neg {
            x: Arc::new(variable_expr.clone()),
            degree_multiple: 1,
        };
        assert_eq!(
        ...
        let mul_expr = SymbolicExpression::<Fr>::Mul {
    ...
    fn test_addition_of_constants() {
        let a = SymbolicExpression::Constant(Fr::new(3));
        let b = SymbolicExpression::Constant(Fr::new(4));
        let result = a + b;
        match result {
            SymbolicExpression::Constant(val) => assert_eq!(val, Fr::new(7)),
    ...
    fn test_subtraction_of_constants() {
        let a = SymbolicExpression::Constant(Fr::new(10));
        let b = SymbolicExpression::Constant(Fr::new(4));
        let result = a - b;
        match result {
            SymbolicExpression::Constant(val) => assert_eq!(val, Fr::new(6)),
    ...
    #[test]
    fn test_negation() {
        let a = SymbolicExpression::Constant(Fr::new(7));
        let result = -a;
        match result {
            SymbolicExpression::Constant(val) => {
                assert_eq!(val, Fr::NEG_ONE * Fr::new(7));
            }
    ...
    fn test_multiplication_of_constants() {
        let a = SymbolicExpression::Constant(Fr::new(3));
        let b = SymbolicExpression::Constant(Fr::new(5));
        let result = a * b;
        match result {
            SymbolicExpression::Constant(val) => assert_eq!(val, Fr::new(15)),
    ...
    fn test_degree_multiple_for_addition() {
        let a =
            SymbolicExpression::Variable::<Fr>(SymbolicVariable::new(Entry::Main { offset: 0 }, 1));
        let b =
            SymbolicExpression::Variable::<Fr>(SymbolicVariable::new(Entry::Main { offset: 0 }, 2));
        let result = a + b;
        match result {
            SymbolicExpression::Add {
                degree_multiple,
    ...
    fn test_sum_operator() {
        let expressions = vec![
            SymbolicExpression::Constant(Fr::new(2)),
            SymbolicExpression::Constant(Fr::new(3)),
            SymbolicExpression::Constant(Fr::new(5)),
        ];
        let result: SymbolicExpression<Fr> = expressions.into_iter().sum();
        match result {
            SymbolicExpression::Constant(val) => assert_eq!(val, Fr::new(10)),
    ...
    fn test_product_operator() {
        let expressions = vec![
            SymbolicExpression::Constant(Fr::new(2)),
            SymbolicExpression::Constant(Fr::new(3)),
            SymbolicExpression::Constant(Fr::new(4)),
        ];
        let result: SymbolicExpression<Fr> = expressions.into_iter().product();
        match result {
            SymbolicExpression::Constant(val) => assert_eq!(val, Fr::new(24)),
    ...
    fn test_default_is_zero() {
        ...
        let expr: SymbolicExpression<Fr> = Default::default();

        // Verify it matches the zero constant.
        assert!(matches!(
            expr,
            SymbolicExpression::Constant(c) if c == Fr::ZERO
        ));
    }
    ...
    fn test_ring_constants() {
        ...
        assert!(matches!(
            SymbolicExpression::<Fr>::ZERO,
            SymbolicExpression::Constant(c) if c == Fr::ZERO
        ));

        // ONE is a Constant variant wrapping the field's one element.
        assert!(matches!(
            SymbolicExpression::<Fr>::ONE,
            SymbolicExpression::Constant(c) if c == Fr::ONE
        ));

        // TWO is a Constant variant wrapping the field's two element.
        assert!(matches!(
            SymbolicExpression::<Fr>::TWO,
            SymbolicExpression::Constant(c) if c == Fr::TWO
        ));

        // NEG_ONE is a Constant variant wrapping the field's -1 element.
        assert!(matches!(
            SymbolicExpression::<Fr>::NEG_ONE,
            SymbolicExpression::Constant(c) if c == Fr::NEG_ONE
        ));
    }
    ...
    fn test_from_symbolic_variable() {
        ...
        let var = SymbolicVariable::<Fr>::new(Entry::Main { offset: 0 }, 3);

        // Convert to expression.
        let expr: SymbolicExpression<Fr> = var.into();
    ...
    fn test_from_field_element() {
        ...
        let field_val = Fr::new(42);
        let expr: SymbolicExpression<Fr> = field_val.into();
    ...
    fn test_from_prime_subfield() {
        ...
        let prime_subfield_val = <Fr as PrimeCharacteristicRing>::PrimeSubfield::new(7);
        let expr = SymbolicExpression::<Fr>::from_prime_subfield(prime_subfield_val);

        // Verify it produces a constant with the converted value.
        assert!(matches!(
            expr,
            SymbolicExpression::Constant(c) if c == Fr::new(7)
        ));
    }
    ...
    fn test_assign_operators() {
        ...
        let mut expr = SymbolicExpression::Constant(Fr::new(5));
        expr += SymbolicExpression::Constant(Fr::new(3));
        assert!(matches!(
            expr,
            SymbolicExpression::Constant(c) if c == Fr::new(8)
        ));

        // Test SubAssign with constants (should simplify).
        let mut expr = SymbolicExpression::Constant(Fr::new(10));
        expr -= SymbolicExpression::Constant(Fr::new(4));
        assert!(matches!(
            expr,
            SymbolicExpression::Constant(c) if c == Fr::new(6)
        ));

        // Test MulAssign with constants (should simplify).
        let mut expr = SymbolicExpression::Constant(Fr::new(6));
        expr *= SymbolicExpression::Constant(Fr::new(7));
        assert!(matches!(
            expr,
            SymbolicExpression::Constant(c) if c == Fr::new(42)
        ));
    }
    ...
    fn test_subtraction_creates_sub_node() {
        ...
        let a =
            SymbolicExpression::Variable::<Fr>(SymbolicVariable::new(Entry::Main { offset: 0 }, 0));
        let b =
            SymbolicExpression::Variable::<Fr>(SymbolicVariable::new(Entry::Main { offset: 0 }, 1));
    ...
    fn test_empty_sum_returns_zero() {
        ...
        let empty: Vec<SymbolicExpression<Fr>> = vec![];
        let result: SymbolicExpression<Fr> = empty.into_iter().sum();
    ...
    fn test_empty_product_returns_one() {
        ...
        let empty: Vec<SymbolicExpression<Fr>> = vec![];
        let result: SymbolicExpression<Fr> = empty.into_iter().product();
    ...
    fn test_mixed_degree_addition() {
        ...
        let constant = SymbolicExpression::Constant(Fr::new(5));
        ...
        match result {
            SymbolicExpression::Add {
                ...
                assert!(matches!(
                    x.as_ref(),
                    SymbolicExpression::Constant(c) if *c == Fr::new(5)
                ));
